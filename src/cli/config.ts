...
// TOML serialization
// ---------------------------------------------------------------------------
// We use smol-toml for parsing and generation. The schema is flat-ish:
// top-level tables with scalar values. Secrets are written as empty strings
// when unset so the file is self-documenting.
// ---------------------------------------------------------------------------

function serializeToml(cfg: EyesConfig): string {
  const header = "# Generated by eyes init. Edit freely — section.key format.\n";
  return header + stringifyToml(cfg as unknown as Record<string, unknown>);
}

function parseTomlSafe(raw: string): Partial<EyesConfig> {
  return parseToml(raw) as Partial<EyesConfig>;
}
